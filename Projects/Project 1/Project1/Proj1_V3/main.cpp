...
int main(int argc, char** argv) {
    ...
    fstream input,      //Reads in the name and cards of the game 
...
/************************************************************************************/    
    ...
    input.open(nameIn.c_str(), ios::in);
    getline(input, namerdin);               //Name is read in from file to a string
    input.close();
    for (int i=0;i<namerdin.length();i++)   
        player[0].name[i]=&namerdin[i];     //char array from structure is pointed to string
    ...
    unsigned int *hand;   //pointer variable created that points to address of player 1 hand 
    hand=&player[0].hand;
